...
formGameJoin.addEventListener("submit", (event) => {
  ...
  goToOnlineGame(
    joinGameInput.value.length > 0 ? joinGameInput.value : Date.now().toString()
  );
});
formGameCreate.addEventListener("submit", (event) => {
  ...
  const gridSizeUnsafe = parseInt(gridSizeInput.value);

  const playerNumber = playerNumberUnsafe >= 2 ? playerNumberUnsafe : 2; // will take two if Nan (not a number passed)
  const gridSize = gridSizeUnsafe >= 2 ? gridSizeUnsafe : 2; // will take two if Nan (not a number passed)

  if (gameType) {
    goToLocalGame(playerNumber, gridSize);
  } else {
