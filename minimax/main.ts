...
export class Board {
    ...
    constructor(n: number = 0, bords: Bord[][][] = [], tiles: Tile[][] = [], playerNb: boolean = false, scores: [number, number] = [0, 0], depth: number = 0) {
        i++;
        this.index = i;
        this.n = n;
        this.bords = bords;
        ...
        if (this.scores[0] == scores[0] && this.scores[1] == scores[1]) this.playerNb = !this.playerNb;
        this.log(`Constructor new Board ---------------------------------- ${this.index} depth ${this.depth}`);
    ...
    jouer(bordId: Identifier): Board {
        let newBords = cloneDeep(this.bords);
        newBords[bordId.dir][bordId.x][bordId.y].appartenance = this.getJoueurActu();
        return new Board(this.n, newBords, cloneDeep(this.tiles), this.playerNb, this.scores, this.depth + 1,);
    }
...
export function miniMax(board: Board, maxDepth: number, depth: number = 0): path {
    ...
    //console.log(score)
    if (score[0] > board.n ** 2 / 2) { return { coups: [{ dir: 0, x: 69, y: 69 }], value: MAX_NUMBER } };
    if (score[1] > board.n ** 2 / 2) { /*console.log(score)*/; return { coups: [{ dir: 1, x: 69, y: 69 }], value: -MAX_NUMBER } };
...
export function findExtrem(map: Map<Bord, path>, playerNb: boolean): path {
    ...
    //console.log(signe);
    let meilleurPath: path = { coups: [{ dir: 0, x: 42, y: 42 }], value: - signe * MAX_NUMBER };
